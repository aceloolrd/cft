...
def main(read_path: str, target_path: str, save_path: str, bootstrap: bool=True) -> None:
    ...
    """
    print("[INFO] Starting to create credit_card_balance_result")

    ccb = pd.read_csv(read_path)
    target = pd.read_csv(target_path, usecols=['TARGET','SK_ID_CURR'])
    ccb = ccb.merge(target, on = 'SK_ID_CURR')

    binary_features, categorical_features, numeric_features = preprocess_data(ccb, {'TARGET','SK_ID_CURR', 'SK_ID_PREV'})
    result_cols = select_significant_features(ccb, binary_features, categorical_features, numeric_features, bootstrap=bootstrap)
    result_cols.extend(['SK_ID_CURR', 'SK_ID_PREV'])
    
    ccb[result_cols].to_csv(save_path, index=False)
    
    print("[INFO] credit_card_balance_result successfully created")

if __name__ == "__main__":
    read_path = 'E:\\home-credit-default-risk\\credit_card_balance.csv'
    target_path = 'E:\\home-credit-default-risk\\application_train.csv'
    save_path = 'E:\\home-credit-default-risk\\credit_card_balance_result.csv'
